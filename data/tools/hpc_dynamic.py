...
# Internal filename: 'build/lib/biomni/tool/hpc_dynamic.py'
# Bytecode version: 3.11a7e (3495)
# Source timestamp: 2007-03-11 20:23:14 UTC (1173644594)
...
def hpc_search_tools(query: str) -> list[dict[str, Any]]:
    if not is_hpc_enabled():
        ...
        return results
def hpc_run_tool(tool_id: str, command: str, input_files: dict[str, str] | None=None, accelerators: dict[str, int] | None=None) -> dict[str, Any]:
    if not is_hpc_enabled():
        raise HPCAPIError('HPC API is not enabled. Set USE_HPC_API=true in environment.')
    else:
        ...
        return job_status
def hpc_get_job_results(job_id: str, poll: bool=True, timeout: int=3600, poll_interval: int=10) -> dict[str, Any]:
    if not is_hpc_enabled():
        raise HPCAPIError('HPC API is not enabled. Set USE_HPC_API=true in environment.')
    else:
        ...
        if not poll:
            ...
            job_status = hpc_api_wait_for_completion(job_id=job_id, poll_interval_seconds=poll_interval, timeout_seconds=timeout)
...
def hpc_run_and_wait(tool_id: str, command: str, input_files: dict[str, str] | None=None, timeout: int=3600, poll_interval: int=10) -> dict[str, Any]:
    job = hpc_run_tool(tool_id=tool_id, command=command, input_files=input_files)
    return hpc_get_job_results(job_id=job['job_id'], poll=True, timeout=timeout, poll_interval=poll_interval)