...
var TOOL_DEFINITIONS = [
  {
    ...
    function: {
      name: "bash",
      description: "Execute a shell command and return stdout/stderr. Use for running R/Python scripts, bioinformatics tools (samtools, STAR, etc.), package installation, and data processing.",
      parameters: {
        type: "object",
        properties: {
          command: {
  ...
  },
  {
    ...
    function: {
      name: "write_file",
      description: "Create or overwrite a file with the given content. Use for saving scripts, results, or configuration files.",
      parameters: {
        type: "object",
        properties: {
...
function toolReadFile(path, maxLines, offset) {
  ...
  if (!(0, import_fs3.existsSync)(resolved)) return { output: "", error: `File not found: ${resolved}` };
  const lines = (0, import_fs3.readFileSync)(resolved, "utf8").split("\n");
...
function toolWriteFile(path, content) {
  const resolved = (0, import_path3.resolve)(path.replace(/^~/, (0, import_os2.homedir)()));
  const dir = (0, import_path3.dirname)(resolved);
  if (!(0, import_fs3.existsSync)(dir)) (0, import_fs3.mkdirSync)(dir, { recursive: true });
  (0, import_fs3.writeFileSync)(resolved, content, "utf8");
  return { output: `\u2713 Written ${resolved} (${content.length} bytes)` };
}
function toolListDir(path) {
  const resolved = (0, import_path3.resolve)(path.replace(/^~/, (0, import_os2.homedir)()));
  if (!(0, import_fs3.existsSync)(resolved)) return { output: "", error: `Path not found: ${resolved}` };
  const entries = (0, import_fs3.readdirSync)(resolved).map((name) => {
    const full = (0, import_path3.join)(resolved, name);
    const isDir = (0, import_fs3.statSync)(full).isDirectory();
...
function printHelp() {
  console.log(source_default.bold.cyan("\u2500\u2500\u2500 \u670D\u52A1\u5546 / \u6A21\u578B \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"));
  console.log(`  ${source_default.cyan("/provider")} <name>   \u5207\u6362\u670D\u52A1\u5546 (bailian|intranet|custom)`);
  console.log(`  ${source_default.cyan("/model")} <name>      \u5207\u6362\u6A21\u578B`);
  console.log(`  ${source_default.cyan("/models")}             \u5217\u51FA\u5F53\u524D\u670D\u52A1\u5546\u6240\u6709\u53EF\u7528\u6A21\u578B`);
  console.log(`  ${source_default.cyan("/providers")}          \u5217\u51FA\u6240\u6709\u670D\u52A1\u5546`);
  console.log(`  ${source_default.cyan("/connect")} [prov]     \u914D\u7F6E API Key`);
  console.log(`  ${source_default.cyan("/status")}             \u663E\u793A\u5F53\u524D\u914D\u7F6E`);
  console.log();
  ...
  console.log(`  ${source_default.cyan("exit")} / ${source_default.cyan("quit")} / ${source_default.cyan("q")}    \u9000\u51FA`);
  console.log();
}
async function setupApiKey(rl, providerName) {
  const prov = PROVIDERS[providerName];
  console.log(source_default.yellow(`
\u9700\u8981\u4E3A ${prov.name} \u914D\u7F6E API Key`));
  if (providerName !== "custom") {
    console.log(source_default.dim("\u83B7\u53D6 Key: \u5728\u5BF9\u5E94\u670D\u52A1\u5546\u5B98\u7F51\u6CE8\u518C\u8D26\u53F7\u5E76\u7533\u8BF7 API Key"));
  }
  const key = await question(rl, source_default.cyan("  API Key (\u65E0\u9700\u9274\u6743\u53EF\u76F4\u63A5\u56DE\u8F66) \u203A "));
  return key.trim();
}
async function setupCustomProvider(rl) {
  const cfg = loadConfig();
  console.log(source_default.yellow("\n\u914D\u7F6E\u81EA\u5B9A\u4E49 / \u5185\u7F51\u5927\u6A21\u578B\u670D\u52A1"));
  console.log(source_default.dim("\u652F\u6301\u4EFB\u4F55\u517C\u5BB9 OpenAI Chat Completions API \u7684\u670D\u52A1\uFF08vLLM\u3001Ollama\u3001LMStudio\u3001FastChat \u7B49\uFF09\n"));
  const currentUrl = cfg.customUrl ? source_default.dim(` (\u5F53\u524D: ${cfg.customUrl})`) : "";
  const url = await question(rl, source_default.cyan(`  API Base URL (\u5982 http://192.168.1.100:8080/v1)${currentUrl} \u203A `));
  const trimmedUrl = url.trim() || cfg.customUrl || "";
  if (!trimmedUrl) {
    console.log(source_default.red("  URL \u4E0D\u80FD\u4E3A\u7A7A"));
    return;
  }
  const currentModel = cfg.customModel ? source_default.dim(` (\u5F53\u524D: ${cfg.customModel})`) : "";
  const model = await question(rl, source_default.cyan(`  \u6A21\u578B\u540D\u79F0 (\u5982 qwen2.5-72b-instruct, llama3.1-70b)${currentModel} \u203A `));
  const trimmedModel = model.trim() || cfg.customModel || "";
  if (!trimmedModel) {
    console.log(source_default.red("  \u6A21\u578B\u540D\u79F0\u4E0D\u80FD\u4E3A\u7A7A"));
    return;
  }
...
\u2713 \u81EA\u5B9A\u4E49\u670D\u52A1\u5546\u5DF2\u914D\u7F6E`));
  console.log(`  URL:   ${source_default.cyan(trimmedUrl)}`);
  console.log(`  \u6A21\u578B:  ${source_default.cyan(trimmedModel)}`);
  console.log(`  \u8BA4\u8BC1:  ${apiKey ? source_default.green("API Key \u5DF2\u8BBE\u7F6E") : source_default.dim("\u65E0\u9700\u9274\u6743")}`);
  console.log();
}
async function firstRunIfNeeded(rl) {
  const cfg = loadConfig();
  const prov = PROVIDERS[cfg.provider];
  if (prov?.envKey === "") return;
  const hasKey = cfg.apiKeys[cfg.provider] && cfg.apiKeys[cfg.provider].length > 0 || prov?.envKey && process.env[prov.envKey];
  if (!hasKey) {
    console.log(source_default.yellow("\n\u6B22\u8FCE\u4F7F\u7528 BGI CLI\uFF01\u9996\u6B21\u4F7F\u7528\u9700\u8981\u914D\u7F6E AI \u670D\u52A1\u5546\u3002\n"));
    console.log("\u8BF7\u9009\u62E9\u670D\u52A1\u5546:");
    const provList = Object.entries(PROVIDERS);
    provList.forEach(([key, p2], i2) => {
      const note = key === "intranet" ? source_default.dim(" (\u5185\u7F51\uFF0C\u65E0\u9700 Key)") : "";
      console.log(`  ${source_default.cyan(i2 + 1)}) ${p2.name}${note}  ${source_default.dim(`(${key})`)}`);
    });
    console.log();
    const choice = await question(rl, source_default.cyan(`\u9009\u62E9 (1-${provList.length}, \u9ED8\u8BA4 1 \u767E\u70BC) \u203A `));
...
function listSkills(keyword) {
  ...
  if (matched.length === 0) {
    console.log(source_default.yellow(`\u672A\u627E\u5230\u5339\u914D "${keyword}" \u7684 Skill`));
...
async function injectSkill(id, history, injectedSkills, rl, skipConfirm = false) {
  ...
  if (!match) {
    console.log(source_default.red(`\u627E\u4E0D\u5230 Skill: ${id}`));
...
function saveConversation(history, filename) {
  if (history.length === 0) {
    console.log(source_default.yellow("\u5BF9\u8BDD\u5386\u53F2\u4E3A\u7A7A\uFF0C\u65E0\u5185\u5BB9\u53EF\u4FDD\u5B58"));
    return;
  }
  const now = /* @__PURE__ */ new Date();
...
async function handleCommand(input, rl, history, thinkMode, injectedSkills) {
  ...
  switch (cmd?.toLowerCase()) {
    case "provider": {
      ...
      if (!PROVIDERS[arg]) {
        console.log(source_default.red(`\u672A\u77E5\u670D\u52A1\u5546: ${arg}`));
      ...
      saveConfig(cfg);
      console.log(source_default.green(`\u2713 \u5207\u6362\u5230 ${PROVIDERS[arg].name}\uFF0C\u6A21\u578B: ${cfg.model}`));
      break;
    }
    case "model": {
      if (!arg) {
      ...
      cfg.model = arg;
      saveConfig(cfg);
      console.log(source_default.green(`\u2713 \u5207\u6362\u5230\u6A21\u578B: ${arg}`));
      break;
    }
    case "models": {
      const prov = PROVIDERS[cfg.provider];
      console.log(source_default.bold(`${prov.name} \u53EF\u7528\u6A21\u578B:`));
      prov.models.forEach((m2) => {
        const current = m2 === cfg.model ? source_default.green(" \u2190 \u5F53\u524D") : "";
        console.log(`  ${m2}${current}`);
      });
      break;
    }
    case "providers": {
      console.log(source_default.bold("\u53EF\u7528\u670D\u52A1\u5546:"));
      Object.entries(PROVIDERS).forEach(([key, p2]) => {
        const current = key === cfg.provider ? source_default.green(" \u2190 \u5F53\u524D") : "";
        const noKey = p2.envKey === "" ? source_default.dim(" (\u65E0\u9700 Key)") : "";
        console.log(`  ${source_default.cyan(key)}: ${p2.name}${noKey}${current}`);
      });
      break;
    }
    case "connect": {
      ...
      if (!PROVIDERS[provKey]) {
        console.log(source_default.red(`\u672A\u77E5\u670D\u52A1\u5546: ${provKey}`));
        break;
      }
      if (provKey === "custom") {
        await setupCustomProvider(rl);
      ...
      if (apiKey) {
        ...
        saveConfig(cfg);
        console.log(source_default.green(`\u2713 API Key \u5DF2\u4FDD\u5B58`));
      }
      break;
    }
    case "status": {
      ...
      console.log(source_default.bold("\u5F53\u524D\u914D\u7F6E:"));
      console.log(`  \u670D\u52A1\u5546:   ${prov?.name ?? cfg.provider}`);
      console.log(`  \u6A21\u578B:     ${cfg.model}`);
      console.log(`  API Key:  ${hasKey ? source_default.green("\u5DF2\u914D\u7F6E") : source_default.red("\u672A\u914D\u7F6E")}`);
      console.log(`  \u5DE5\u4F5C\u76EE\u5F55: ${process.cwd()}`);
      console.log(`  \u601D\u8003\u6A21\u5F0F: ${thinkMode ? source_default.yellow("\u5F00\u542F (/think)") : source_default.dim("\u5173\u95ED")}`);
      break;
    }
    case "clear":
      console.log(source_default.dim("\u5BF9\u8BDD\u5386\u53F2\u5DF2\u6E05\u7A7A"));
...
`));
    ...
    case "think": {
      ...
      if (val === "on" || val === "1" || val === "true") {
        console.log(source_default.yellow("\u2713 \u601D\u8003\u6A21\u5F0F\u5DF2\u5F00\u542F \u2014 \u6BCF\u6761\u6D88\u606F\u5C06\u9644\u52A0 /think \u524D\u7F00 (Qwen3)"));
        return { thinkMode: true };
      } else if (val === "off" || val === "0" || val === "false") {
        console.log(source_default.dim("\u2713 \u601D\u8003\u6A21\u5F0F\u5DF2\u5173\u95ED"));
        return { thinkMode: false };
      } else {
        const next = !thinkMode;
        console.log(next ? source_default.yellow("\u2713 \u601D\u8003\u6A21\u5F0F\u5DF2\u5F00\u542F \u2014 \u6BCF\u6761\u6D88\u606F\u5C06\u9644\u52A0 /think \u524D\u7F00 (Qwen3)") : source_default.dim("\u2713 \u601D\u8003\u6A21\u5F0F\u5DF2\u5173\u95ED"));
...
`));
    ...
    case "cd": {
      ...
      console.log(source_default.green(`\u2713 \u5DE5\u4F5C\u76EE\u5F55\u5DF2\u5207\u6362: ${process.cwd()}`));
      break;
    }
    case "cwd":
      console.log(`\u5F53\u524D\u5DE5\u4F5C\u76EE\u5F55: ${source_default.cyan(process.cwd())}`);
      break;
    case "tools":
    ...
    case "help":
      ...
      break;
    default:
      console.log(source_default.yellow(`\u672A\u77E5\u547D\u4EE4: /${cmd}\u3002\u8F93\u5165 /help \u67E5\u770B\u5168\u90E8\u547D\u4EE4`));
  }
  return {};
}
async function main() {
  ...
  while (true) {
    ...
    try {
      input = await question(rl, thinkIndicator + source_default.blue("\u4F60 \u203A "));
    } catch {
      break;
    }
...
main().catch((err) => {
  console.error(source_default.red("Fatal:", err instanceof Error ? err.message : String(err)));
  process.exit(1);
});
